...
We will use the synthetic data set included with the package and call it `dat` to illustrate the use of the package. In this data set, `tss_0` is the continuous exposure and represents a count of trauma symptoms, `sfs8p_3`, is the outcome variable and measures substance frequency at 3-month follow-up, and `sfs8p_0` - the substance frequency at baseline, `treat` - the treatment group, and `sati_0` - substance abuse treatment index at baseline are covariates. These covariates are potential confounders of `tss_0` and `sfs8p_3` and were generated to mimic the confounders in the real dataset. Load the data using the following code.
...
The function includes several optional arguments: `n.trees`, `interaction.depth`, and `shrinkage` are parameters for the `gbm` model and `verbose` and `sampw`.
...
For those familiar with the `ps()` function in `twang`, unlike the `ps()` function in `twang`, `ps.cont()` does not have arguments for `estimand` or `perm.test.iters` as these are not relevant for the continuous exposure context.
...
The results indicate that for each additional trauma symptom, substance frequency increases by 0.002, which is not statistically significant, $t=0.04, p = .968$.
